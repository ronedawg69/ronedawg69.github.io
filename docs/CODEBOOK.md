...
## Lesson 1 weather-card readings

- **Temperature** is modelled air temperature two metres above ground at the
  generic London point; it is not a bicycle-mounted or street-level measurement.
- **Feels like** is the provider's apparent-temperature estimate, combining factors
  such as temperature, wind, humidity, and sunlight to approximate human sensation.
- **Precipitation** is the combined modelled precipitation total for the preceding
  15-minute interval, expressed in millimetres; `0` means none in that interval,
  not necessarily that the whole day has been dry.
- **Wind** shows modelled wind speed ten metres above ground and the direction the
  wind is coming **from**. Degrees run clockwise from north: 0°/360° is north, 90°
  east, 180° south, and 270° west.
- **Weather code** is a WMO category number. The card deliberately shows the raw
  value in the API response, then JavaScript translates it through the official
  mapping into a readable condition. An unexpected code is displayed honestly as
  `Unknown condition (code …)` rather than being guessed.
- **Data interval** is the number of seconds represented by the current record;
  `900` seconds equals 15 minutes.
- **Weather time** comes from the provider's weather record. **Last updated** is
  when the visitor's browser successfully received and displayed the response.

### WMO interpretation mapping supplied: 2026-09-08

Ronan supplied a screenshot of the current “WMO Weather interpretation codes
(WW)” table from <https://open-meteo.com/en/docs>. The implementation maps codes
individually to the descriptions represented by the official grouped rows:

- `0`: clear sky; `1`, `2`, `3`: mainly clear, partly cloudy, overcast.
- `45`, `48`: fog, depositing rime fog.
- `51`, `53`, `55`: light, moderate, dense drizzle; `56`, `57`: light, dense
  freezing drizzle.
- `61`, `63`, `65`: slight, moderate, heavy rain; `66`, `67`: light, heavy
  freezing rain.
- `71`, `73`, `75`: slight, moderate, heavy snow fall; `77`: snow grains.
- `80`, `81`, `82`: slight, moderate, violent rain showers; `85`, `86`: slight,
  heavy snow showers.
- `95`: slight or moderate thunderstorm; `96`, `99`: thunderstorm with slight,
  heavy hail. The supplied table notes that hail forecasting is available only in
  Central Europe.

`weatherCondition(code)` performs this translation. This is calculated/translated
display data, not text returned directly in the response. `intervalInMinutes`
divides the fetched seconds by 60, so `900` becomes `15 minutes`.
