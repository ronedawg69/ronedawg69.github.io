...
- **Status:** Readability exercise completed and implemented; deployment, Network trace, and failure check pending
- **Rule:** Do not mark Lesson 1 complete or begin Lesson 2 until Ronan verifies the refined live display, traces one displayed value to the live JSON, and verifies the failure state.
...
### Weather readability exercise — 2026-09-08

| Question | Ronan's answer | Assessment / misconception | Follow-up |
| --- | --- | --- | --- |
| How is 900 seconds converted to minutes? | Divide by 60. | Correct: `900 / 60` produces `15`. | Implemented in `intervalInMinutes`. |
| Is “Partly cloudy” fetched directly or translated? | Translated by JavaScript. | Correct: the API supplies code `2`; our code maps it using the official WMO table. | Trace raw code and displayed wording during the Network check. |

## Provisional curriculum progress

| Lesson | Status |
| --- | --- |
| 1. First real London weather request | In progress — readability refinement implemented; deployment, Network trace, and failure check pending |
